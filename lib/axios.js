...
const api = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_URL || "https://api.myworkonline.in/api/v1", 
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
       ...
       await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL || "https://api.myworkonline.in/api/v1"}/users/refresh-token`, 
          {}, 
