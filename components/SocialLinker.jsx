  ...
  return (
    <div className="space-y-8">
      {linkedAccounts.length > 0 && (
          <div className="space-y-4">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  <CheckCircleIcon className="w-6 h-6 text-green-500" /> Linked Accounts
              </h3>
              <div className="grid gap-4">
                  {linkedAccounts.map((acc, i) => (
                      <div key={i} className="flex items-center justify-between p-5 bg-white rounded-xl border border-slate-200 shadow-sm">
                          <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl ${acc.platform === 'Moj' ? 'bg-orange-50 text-orange-500' : 'bg-purple-50 text-purple-500'}`}>
                                  {acc.platform === 'Moj' ? '🔥' : '📷'}
                              </div>
                              <div className="overflow-hidden">
                                  <h4 className="font-bold text-slate-900">{acc.platform}</h4>
                                  <a href={acc.profileUrl} target="_blank" className="text-sm text-violet-600 hover:underline truncate block max-w-[200px]">@{acc.username || "user"}</a>
                              </div>
                          </div>
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
        <h3 className="text-2xl font-extrabold text-slate-900 mb-6">Link New Account</h3>
        {step === 1 ? (
            <div className="space-y-5">
                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Select Platform</label>
                    <select value={platform} onChange={(e) => setPlatform(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-4 rounded-xl outline-none focus:ring-2 focus:ring-violet-500 font-medium">
                        <option value="Moj">Moj App</option>
                        <option value="ShareChat">ShareChat</option>
                    </select>
                </div>
                <button onClick={generateCode} disabled={loading} className="w-full py-4 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl transition shadow-lg shadow-violet-200">
                    {loading ? "Generating..." : "Next: Get Verification Code"}
                </button>
            </div>
        ) : (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                <div className="bg-blue-50 border border-blue-100 p-5 rounded-xl text-center">
                    <p className="text-xs text-blue-500 uppercase font-bold mb-3">Step 1: Copy Code to Bio</p>
                    <button onClick={() => {navigator.clipboard.writeText(verificationCode); toast.success("Copied!")}} className="flex items-center gap-3 justify-center bg-white w-full py-3 rounded-lg border border-blue-200 hover:border-blue-400 transition group cursor-pointer">
                        <span className="text-2xl font-mono font-black text-slate-800 tracking-widest group-hover:text-blue-600">{verificationCode}</span>
                        <CopyIcon className="w-5 h-5 text-slate-400 group-hover:text-blue-500" />
                    </button>
                    <p className="text-xs text-slate-500 mt-3">Edit your {platform} profile bio and paste this code.</p>
                </div>

                <div>
                    <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Step 2: Profile Link</label>
                    <input type="text" placeholder="https://..." value={url} onChange={(e) => setUrl(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 p-4 rounded-xl outline-none focus:ring-2 focus:ring-green-500 transition" />
                </div>

                <div className="flex gap-4">
                    <button onClick={() => setStep(1)} className="px-6 py-4 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200">Back</button>
                    <button onClick={verifyAccount} disabled={loading} className="flex-1 py-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl transition shadow-lg shadow-green-200">
