...
export default function SettingsPage() {
  ...
  const saveSettings = async (type) => {
    setLoading(true);
    try {
        if(type === 'reward') await api.post("/admin/settings", { key: "reward_per_task", value: reward });
        if(type === 'announcement') await api.post("/admin/announcement", announcement);
        toast.success("Saved!");
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex gap-4">
            <div className="flex-1">
                ...
                <input type="number" value={reward} onChange={(e) => setReward(e.target.value)} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-violet-500 font-bold text-slate-900" />
            </div>
            <div className="flex items-end">
                <button onClick={() => saveSettings('reward')} disabled={loading} className="px-8 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition">Save</button>
            </div>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
        ...
        <div className="space-y-4 relative z-10">
            <div>
                ...
                <input type="text" value={announcement.message} onChange={(e) => setAnnouncement({...announcement, message: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-violet-500" placeholder="Announcement text..." />
            </div>
            ...
            <div className="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <input type="checkbox" className="w-6 h-6 accent-violet-600" checked={announcement.isActive} onChange={(e) => setAnnouncement({...announcement, isActive: e.target.checked})} />
            </div>

            <button onClick={() => saveSettings('announcement')} disabled={loading} className="w-full py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl transition shadow-lg shadow-violet-200">Update Broadcast</button>
        </div>
      </div>
    </div>
  );
