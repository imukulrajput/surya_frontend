  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900">Daily Tasks</h1>
            <p className="text-slate-500">Complete tasks to earn rewards.</p>
          </div>
          <div className="bg-white px-6 py-4 rounded-xl border border-slate-100 shadow-sm text-right">
             <p className="text-xs text-slate-400 uppercase font-bold tracking-wider">Today's Earnings</p>
             <p className="text-green-600 font-black text-3xl">₹ {tasks.filter(t => t.status === 'Approved').length * 2.5}</p>
          </div>
      </div>

      {hasLinkedAccount ? (
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
           <label className="text-xs text-slate-400 uppercase font-bold mb-3 block">Select Active Account</label>
        ...
        <div className="bg-blue-50 border border-blue-200 p-6 rounded-2xl flex items-center justify-between gap-4">
            <div>
                <h3 className="font-bold text-blue-900 text-lg">View Only Mode</h3>
                <p className="text-sm text-blue-700">Link your social account to unlock submissions.</p>
            </div>
            <Link href="/verify"><button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition shadow-lg shadow-blue-200">Link Account</button></Link>
        </div>
      )}

      <div className="space-y-6">
        {tasks.map((task, index) => (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} key={task._id} 
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    <div className="w-full md:w-48 shrink-0 bg-slate-50 rounded-xl p-6 flex flex-col items-center justify-center border border-slate-200 text-center">
                        <div className="mb-4 scale-125"><DriveIcon /></div>
                        <a href={task.videoUrl} target="_blank" className="w-full py-2 bg-white border border-slate-300 text-slate-700 hover:text-violet-600 hover:border-violet-400 text-xs font-bold rounded-lg transition shadow-sm">
                            Download File
                        </a>
                    </div>
                    ...
                    <div className="flex-1 space-y-4">
                        <div className="flex justify-between items-start">
                            <h3 className="font-bold text-xl text-slate-800">Task #{index + 1}: {task.title}</h3>
                            <span className="bg-yellow-100 text-yellow-700 border border-yellow-200 text-xs px-3 py-1 rounded-full font-mono font-bold">₹{task.rewardAmount}</span>
                        </div>

                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex justify-between items-center gap-3">
                            <p className="text-sm text-slate-600 truncate flex-1 font-mono">{task.caption}</p>
                            <button onClick={() => {navigator.clipboard.writeText(task.caption); toast.success("Copied!")}} className="text-xs text-violet-600 hover:text-violet-800 font-bold uppercase tracking-wide">Copy</button>
                        </div> 

                        {!hasLinkedAccount ? (
                            <div className="bg-slate-100 p-4 rounded-lg text-center text-sm text-slate-500 font-medium">Link account to submit</div>
                        ) : (
                            <>
                                {task.status === "Approved" ? (
                                    <div className="p-3 bg-green-100 text-green-700 text-sm font-bold rounded-lg flex items-center gap-2">✓ Task Approved</div>
                                ) : task.status === "Pending" ? (
                                    <div className="p-3 bg-yellow-100 text-yellow-700 text-sm font-bold rounded-lg flex items-center gap-2">⏳ Pending Approval</div>
                                ) : (
                                    <>
                                        {task.status === "Rejected" && (
                                            <div className="text-xs text-red-600 bg-red-50 p-2 rounded mb-2">Reason: {task.adminComment}</div>
                                        )}
                                        <div className="flex gap-2">
                                            <input type="text" placeholder={`Paste ${selectedAccount?.platform} link...`} 
                                                className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:ring-2 focus:ring-violet-500 outline-none transition"
                                                value={proofInputs[task._id] || ""} onChange={(e) => setProofInputs({...proofInputs, [task._id]: e.target.value})}
                                            />
                                            <button onClick={() => handleSubmit(task)} disabled={submitting === task._id} 
                                                className="px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl text-sm transition shadow-lg shadow-violet-200 disabled:opacity-50">
                                                {submitting === task._id ? '...' : 'Submit'}
                                            </button>
                                        </div>
                                    </>
